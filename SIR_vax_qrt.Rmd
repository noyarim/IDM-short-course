...
We set the total time steps and run the SIR modelwith imperfect protection from vaccination. Because each compartment is stratified, we computed the total S, I, R compartment
```{r}
T_end <- 500 #run model for 500 time steps (e.g. months)
times <- seq(0, T_end, by = 1) #runs the model for 500 time steps (e.g. months), and computes output at each time step 
