...
## Adds vaccination and waning immunity to the basic SIR model (closed population)
This code is to illustrate how to model vaccination in an SIRS model without demography (no births or deaths) but with waning immunity. The backward flow from R to S at rate omega represents loss of immunity over time (both natural and vaccine-induced).

First, let's look at the case where we assume that vaccine provides perfect protection from infection, with the same rate of immunological waning as natural infection. Here, we defined the ode function with S,I,R compartment. In the differential equation, we have a direct flow from S to R, indicating those who are vaccinated and protected from infection, and a backward flow R -> S at rate omega for waning immunity.
```{r}
...
We then define parameters and starting compartment sizes for ode solver. Mu is vaccination rate. Omega is the rate of waning immunity (1/duration of immunity).
```{r}
...
We set the total time steps and run the SIRS model with perfect protection from vaccination.
```{r}
...
```
Here, vaccination provides less difference than we might expect because of the fast waning.
However, this model structure forces us to assume that vaccination provides the same duration of immunological protection as natural infection, which is often unrealistic.
Furthermore, vaccination usually doesn't provide perfect protection from infection. So let's see how we can model imperfect protection with vaccination. Another consideration to add is whether or not those who are infected after vaccination are less infectious than those who are infected without vaccination. So here we start with the model that assumes imperfect protection and no change in infectiousness with vaccination. This structure
also allows us to specify different waning rates for vaccine-induced (omega_vx) and natural (omega) immunity. We set them equal in the params here, but they can be varied independently.

Below, we defined the ode function with the S compartment stratified by vaccine status, but a single I and a single R compartment. Because vaccinated and unvaccinated infected individuals are equally infectious in this version (ip1), splitting I would not change the dynamics - so we keep a single I. (This does mean we can't report breakthrough infections separately here; see ip2 below for a version that splits I and lets us report breakthrough infections.) Non-vaccinated susceptibles move to the vaccinated susceptible compartment at rate mu. (1-alpha) indicates reduced susceptibility for vaccinated susceptibles and the higher alpha is, the lower the susceptibility would be. Natural-infection-induced immunity wanes from R -> S_NotV at rate omega, while vaccine-induced immunity wanes from S_V -> S_NotV at rate omega_vx.
```{r}
...
And here is the model that assumes imperfect protection and reduced infectiousness with vaccination. Now we apply separate beta values (beta.1 and beta.2) to I_NotV and I_V. Beta.2 is calculated as a fraction of beta.1 (fraction: mbeta). Because infectiousness now depends on vaccine status, we keep I_NotV and I_V as separate compartments; as a bonus, this also lets us report breakthrough infections (I_V over time).
```{r}
# 1. Define model function
...
We then define parameters and starting compartment sizes for ode solver. Mu is vaccination rate. Omega is rate of waning of natural-infection-induced immunity. Omega_vx is the rate of waning of vaccine-induced immunity (set equal to omega here, but can be varied independently). mbeta is set to 1 if no change in infectiousness and a value less than 1 if we assume reduced infectiousness. alpha is vaccine effectiveness. ip1 uses a single I compartment; ip2 splits I by vaccine status, so each model has its own starting state vector.
```{r}
# 2. Define parameters and starting compartment sizes
params <- c(beta = 0.5, #effective contact rate (aka transmission rate)
...
# initial states (ip1: single I; ip2: split I by vaccine status)
...
We set the total time steps and run both versions of the SIRS model with imperfect protection from vaccination. Each model uses its corresponding state vector. We compute total S in both; for ip2 we also sum I_V + I_NotV to get total I.
```{r}
T_end <- 500 #run model for 500 time steps (e.g. months)
times <- seq(0, T_end, by = 1) #runs the model for 500 time steps (e.g. months), and computes output at each time step
...
# Calculate total S (I is already a single column for ip1; for ip2 we also sum I_V + I_NotV)
...
In the next set of codes, we are going to perform sensitivity analysis on (1) vaccination rate, (2) vaccine effectiveness, (3) the ratio in effective contact rate between vaccinated and non-vaccinated individuals, and (4) the rate of waning immunity (varying omega and omega_vx together in lockstep), to see how varying those parameter values changes projected epidemics. (1), (2), and (4) will be done in SIRS_Vax_ip1 and (3) will be done in SIRS_Vax_ip2.
...
How does the projected epidemic change as we decrease the ratio of effective contact rate? This sensitivity uses SIRS_Vax_ip2, whose stratified I compartments also let us report breakthrough infections (I_V over time).
```{r}
...
How does changing the rate of waning immunity affect the projected epidemic? Larger values mean faster loss of immunity, which can sustain transmission and produce repeated waves in a closed population. Here we vary omega (natural-infection waning) and omega_vx (vaccine-induced waning) together in lockstep.
```{r}
...
ggplot(omega_out_all_t) +
  ...
  facet_wrap(.~omega)+
  theme_bw()
# Plot prevalence of I (I/N) on a single panel for easy comparison across omegas
