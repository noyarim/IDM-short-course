###############################################################################################

#SI model with contact tracing (adds treatment + contact tracing to the basic SI model)

###############################################################################################
library(deSolve)
library(ggplot2)
library(reshape2)
library(dplyr)

## SI model with contact tracing ##
# Tx compartment indicates infected individuals who are under treatment and contact tracing.

#1. Define model function
SI_CC<-function(t, state, params) {
  with(as.list(c(state, params)),{
    N = S + I + Tx

    #SI w/o demography equations from lecture
    dS <- -beta*S*I/N + rt*Tx
    dI <- beta*S*I/N - si*I - c*b*Tx
    dTx <- si*I + c*b*Tx - rt*Tx
    # Cumulative incidence
    dC <- beta*S*I/N
    # return the rates of change as a list
    list(c(dS, dI, dTx, dC))
  })
}

#2. Define parameters and starting compartment sizes
params <- c(beta = 0.5, #effective contact rate (aka transmission rate)
                rt = 0.1, # 1/duration of treatment and tracing
                si = 0.1, # rate of seeking treatment
                c = 0.05, # rate of contact tracing
                b = 0.05# probability that a traced individual is infected
)
# Initial state
state <- c(S = 99999, #population of 100,000, 1 person starts of infected
             I = 1,
             Tx = 0,
             C = 0)

#3. Run the SI model
T_end <- 500 #run model for 500 time steps (e.g. months)
times <- seq(0, T_end, by = 1) #runs the model for 500 time steps (e.g. months), and computes output at each time step

#Run the base-case
output.ct <- as.data.frame(ode(y = state, times = times, func = SI_CC, parms = params))

# Plot S, I, Tx compartments over time
output.ct.t <- melt(output.ct %>% select(-C), id.vars="time")
ggplot(output.ct.t) +
  geom_line(aes(time,value,color=variable))+
  theme_bw()

#4. Sensitivity analysis on contact tracing rate - how does it affect the dynamics?
c_list <- c(0,0.3,0.5) # a vector of contact tracing rate
output_dt <- data.frame() # empty data to save outcomes

for (this_c in c_list){
  temp <- params
  # Replace contact tracing rate with the next value
  temp['c'] <- this_c
  # Run ode solver
  this_output <- data.frame(ode(y = state, times = times, func = SI_CC, parms = temp))
  # Record current value of c
  this_output$c = as.character(this_c)
  # Stack the result
  output_dt <- rbind(output_dt, this_output)

}

# Plot the results with varying contact tracing rate
ggplot(output_dt)+
  geom_line(aes(x=time, y=I, color=c, group=c))+
  ylab("Infected")+
  xlab("Time")+
  theme_bw()
