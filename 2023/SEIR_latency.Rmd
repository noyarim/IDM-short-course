...
## Adds latent state to the basic SIR model
This code is to illustrate how to model disease latency in SIR model.

First, we defined the ode function with S,E,I,R compartment. E compartment indicates those who were exposed to the infectious agent but have not developed infectiousness In the differential equation, rate of progressing from E to I is internally calculated based on the latent period (sigma = 1/t_lat). Note that E does not infect others (i.e. number of new infections is still beta*S*I/N but not beta*S*(E+I)/N).
...
We then define parameters and starting compartment sizes for ode solver. t_lat the duration of latency before getting infectious. In this example, we assumed no waning immunity (omega=0) but this assumption can be easily relaxed. 
```{r}
...
We set the total time steps and run the SEIR model. 
```{r}
...
Let's look at how the number of infections change with varying duration of latency.
```{r}
...
Plot the number of infection (size of I compartment) with varying latent period. Here, we plotted only I to assess the size of population who are infectious but if you are interested in 'true' number of infections, you can compute sum of I and E compartments. With longer disease latency, how does the epidemic curve change?
...
ggplot(output_dt)+
  ...
  theme_bw()
```
We can further look into the relationship between the timing of epidemic peak and latent period
```{r}
...
Plot the result on the relationship between epidemic peak and latent period. 
```{r, echo=FALSE}
ggplot(output_dt)+
