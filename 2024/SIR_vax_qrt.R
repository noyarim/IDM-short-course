...
#SIR model with vaccination and quarantine (with demography)
...
OpenSIR_Vax_pp<-function(t, state, params) {
...
# Run the base-case
output.vax.pp <- ode(y = state.pp, times = times, func = OpenSIR_Vax_pp, parms = params)
...
#1. Define model function
OpenSIR_Vax_ip1<-function(t, state, params) {
  with(as.list(c(state, params)),{
...
OpenSIR_Vax_ip2<-function(t, state, params) {
  with(as.list(c(state, params)),{
    N = S_NotV + S_V + I_NotV + I_V + R_NotV + R_V
...
# Run ODE solver
output.vax.ip1 <- ode(y = state.ip, times = times, func = OpenSIR_Vax_ip1, parms = params)
output.vax.ip2 <- ode(y = state.ip, times = times, func = OpenSIR_Vax_ip2, parms = params)
# Calculate S, I, and R
...
# (1) and (2) use OpenSIR_Vax_ip1; (3) uses OpenSIR_Vax_ip2.
...
#1. Define model function
OpenSIR_Qrtn<-function(t, state, params) {
  with(as.list(c(state, params)),{
    N = S + I + Q + R
...
times <- seq(0, T_end, by = 1) #runs the model for 500 time steps (e.g. months), and computes output at each time step
# Run ODE solver
output.qrtn <- ode(y = state.q, times = times, func = OpenSIR_Qrtn, parms = params)
