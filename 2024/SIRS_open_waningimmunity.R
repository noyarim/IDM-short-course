...
#SIRS model with demography (open population) - waning immunity + births and deaths
...
## SIRS model (a model with waning immunity), with births and deaths ##
# Backward flow from R to S indicates those who lost infection-induced immunity over time.

#1. Define model function
OpenSIRS<-function(t, state, params) {
...
output <- ode(y = state, times = times, func = OpenSIRS, parms = params)
...
for (this_omega in omega_list){
  ...
  # Solve ODE
  this_output <- as.data.frame(ode(y = state, times=times, func=OpenSIRS, parms = params))
